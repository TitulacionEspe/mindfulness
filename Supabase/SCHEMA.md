...
| created_at         | TIMESTAMPTZ | default NOW()                                               |
| updated_at         | TIMESTAMPTZ | default NOW(); trigger `set_updated_at_thought_entries_p`   |
...
- Índices: `idx_profiles_role`, `idx_patient_settings_patient`, `idx_routines_category`, `idx_routine_assets_routine`, `idx_activity_sessions_patient`, `idx_self_assessments_patient`, `idx_self_assessments_session_context_unique`, `idx_thought_entries_patient_created_desc`, `idx_assignments_composite`, `idx_sleep_logs_patient_date`.
...
- `activity_sessions`: incluye politica de `UPDATE` del propio paciente para cerrar sesiones completadas.
- `self_assessments`: indice unico parcial `(session_id, context)` para evitar duplicar pre/post por sesion.
- `thought_entries`: políticas explícitas `SELECT/INSERT/UPDATE/DELETE` con `USING` + `WITH CHECK`; sólo el dueño (`auth.uid() = patient_id`) puede operar sobre sus entradas.
